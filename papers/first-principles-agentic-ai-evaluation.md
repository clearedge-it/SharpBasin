...
Government evaluators need to assess three dimensions simultaneously: **task completion** (did the agent achieve the objective?), **behavioral safety** (did it respect defined boundaries?), and **process fidelity** (did it use appropriate tools and reasoning paths?). No single evaluation framework addresses all three. **SharpBasin** — ClearEdge's AI assessment platform — addresses this through its **BasinBench** evaluation suite, which composes three complementary open-source frameworks: UK AISI's **Inspect** for structured evaluation workflows, Microsoft's **PyRIT** for adversarial red-teaming, and NIST's **Dioptra** for AI Risk Management Framework-aligned trustworthiness tracking. The harness is anchored by three first principles.
