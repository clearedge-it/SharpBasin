...
- The full Inspect evaluation pipeline has been validated end-to-end with live API calls against `anthropic/claude-sonnet-4-6` as grader. Evaluation runs on nano (4 samples, 34s) and canary (25 samples, 1m43s) tiers confirmed that dataset loading, model invocation, three-dimensional grading, and log persistence operate correctly across both the `dimensional_scorer` (per-dimension mean/stderr) and `composite_scorer` (aggregate accuracy) variants.
